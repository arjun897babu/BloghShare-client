...
const BlogCardList = () => {
    ...
    const blogDivListRef = useRef<HTMLDivElement>(null)
    ...
    useEffect(() => {
        if (blogs && blogs.length > 0 && search.trim()) {
            blogDivListRef.current?.scrollIntoView({ behavior: "smooth" });
        }
    }, [blogs]);
    ...
    useEffect(() => {
        ...
        fetchData()
    }, [location.pathname, pageNumber, search])
            ...
            <div className={`${isHome ? 'mt-28' : 'mt-2'} mb-5`}>
...

                {
                    blogs && blogs.length === 0 ?
                        (<svg className="mx-auto" xmlns="http://www.w3.org/2000/svg" width="128" height="124" viewBox="0 0 128 124" fill="none">
